var soundMaker;
(function (soundMaker) {
    var $ = document, inptList = $.getElementsByTagName('input'), audioSound = $.getElementById('audioSound'), btnPlay = $.getElementById('play');
    ...
    var getSoundData = function (val) {
        var xhttp = new XMLHttpRequest(), data, cls;
            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                for (var j = 0; j < data.soundData.length; j++) {
                    ...
                    if (data.soundData[j].char === val) {
                        var html = '<audio class="aud" id="aud', $ = { j: j };
                        '" nocontrols ><source src = "';
                        $;
                        {
                            data.soundData[j].audio;
                        }
